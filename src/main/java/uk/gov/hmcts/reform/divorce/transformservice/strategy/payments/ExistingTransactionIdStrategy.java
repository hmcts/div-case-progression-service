...
public class ExistingTransactionIdStrategy implements PaymentStrategy {
    ...
    public List<PaymentCollection> getCurrentPaymentsList(Payment newPayment, List<PaymentCollection> existingPayments) {
        existingPayments.removeIf(payment -> payment.getValue().getPaymentTransactionId().equals(newPayment.getPaymentTransactionId()));
    ...
    public boolean accepts(Payment newPayment, List<PaymentCollection> existingPayments) {
        return Objects.nonNull(existingPayments) && existingPayments.stream()
                .anyMatch(payment -> payment.getValue().getPaymentTransactionId().equals(newPayment.getPaymentTransactionId()));
    }
