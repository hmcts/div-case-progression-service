...
public class BaseCcdClientConfiguration implements CcdClientConfiguration {
    ...
    //citizen urls
    private static final String CREATE_CASE_URL_FORMAT = "%s/citizens/%d/jurisdictions/%s/case-types/%s/event-triggers/create/token?ignore-warning=true";
    private static final String SUBMIT_CASE_URL_FORMAT = "%s/citizens/%d/jurisdictions/%s/case-types/%s/cases?ignore-warning=true";
    private static final String START_CASE_EVENT_URL_FORMAT = "%s/citizens/%d/jurisdictions/%s/case-types/%s/cases/%d/event-triggers/%s/token?ignore-warning=true";
    private static final String CREATE_CASE_EVENT_URL_FORMAT = "%s/citizens/%d/jurisdictions/%s/case-types/%s/cases/%d/events?ignore-warning=true";
    //update caseworker urls
    private static final String START_CASE_EVENT_URL_CASEWORKER_FORMAT = "%s/caseworkers/%d/jurisdictions/%s/case-types/%s/cases/%d/event-triggers/%s/token?ignore-warning=true";
    private static final String CREATE_CASE_EVENT_URL_CASEWORKER_FORMAT = "%s/caseworkers/%d/jurisdictions/%s/case-types/%s/cases/%d/events?ignore-warning=true";
    ...
    public String getStartEventUrl(String encodedJwtToken, Long caseId, String eventId) {
        ...
        String url = isCaseWorkerUser(jwt.getData()) ? START_CASE_EVENT_URL_CASEWORKER_FORMAT : START_CASE_EVENT_URL_FORMAT;
    ...
    public String getCreateCaseEventUrl(String encodedJwtToken, Long caseId) {
        Jwt jwt = getJwt(encodedJwtToken);
        String url = isCaseWorkerUser(jwt.getData()) ? CREATE_CASE_EVENT_URL_CASEWORKER_FORMAT : CREATE_CASE_EVENT_URL_FORMAT;
        return String.format(url, ccdBaseUrl, jwt.getId(), jurisdictionId, caseTypeId, caseId);
    }

    private boolean isCaseWorkerUser(String group) {
        return Optional.ofNullable(group).isPresent()
                && Arrays.asList(group.split(",")).stream()
