...
public class CaseUpdateFunctionalTest {
    ...
    public void shouldReturnCaseIdWhenUpdatingCcdWithPayment() throws Exception {
        ...
        ResponseEntity<CCDResponse> response = restTemplate.postForEntity("/transformationapi/version/1/updateCase/987654321", entity, CCDResponse.class, new HashMap<>());
    ...
    public void shouldReturnNotFoundWhenCaseIdIsNotFound() throws Exception {
        ...
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.OK);
        assertThat(body.getError()).isEqualTo("Request Id : 1234567 and Exception message : 404 Not Found, Exception response body: ");
    ...
    public void shouldHandleServiceAuthUnknownMicroService() throws Exception {
        ...
        String requestBody = FileUtils.readFileToString(new File(getClass().getResource("/fixtures/divorce/update-request-body.json").toURI()), Charset.defaultCharset());
    ...
    private void eventStartStub() throws Exception {
        String eventStartResponseBody = FileUtils.readFileToString(new File(getClass().getResource("/fixtures/ccd/event-start-200-response.json").toURI()), Charset.defaultCharset());

        String eventTypeId = "paymentMade";

        String url = String.join("/", "citizens", USER_ID, "jurisdictions", JURISDICTION, "case-types", CASE_TYPE_ID, "cases", CASE_ID, "event-triggers", eventTypeId, "token");
    ...
    private void eventStartStubWithFilePath(final String filePath) throws Exception {
        String eventStartResponseBody = FileUtils.readFileToString(new File(getClass().getResource(filePath).toURI()), Charset.defaultCharset());
    ...
    private void caseUpdateStub(final String filePath) throws Exception {
        ...
        String responseBody = FileUtils.readFileToString(new File(getClass().getResource("/fixtures/ccd/case-event-creation-201-response.json").toURI()), Charset.defaultCharset());

        String url = String.join("/", CCD_CITIZENS_ENDPOINT, USER_ID, "jurisdictions", JURISDICTION, "case-types", CASE_TYPE_ID, "cases", CASE_ID, "events?ignore-warning=true");
    ...
    private void caseUpdateFailedStub() throws Exception {
        String requestBody = FileUtils.readFileToString(new File(getClass().getResource("/fixtures/ccd/case-event-creation-request-body.json").toURI()), Charset.defaultCharset());
        JSONObject requestBodyWithCreatedDate = populateCreatedDate(requestBody);
